...
class SppagebuilderAddonJs_slideshow extends SppagebuilderAddons
{
	...
	public function render()
	{
		...
		$aria_prev = Text::_('COM_SPPAGEBUILDER_ARIA_PREVIOUS');
        $aria_next = Text::_('COM_SPPAGEBUILDER_ARIA_NEXT');

		$data_aria_prev = ' data-aria-prev="' . $aria_prev . '" ';
		$data_aria_next = ' data-aria-next="' . $aria_next . '" ';
		...
		if (isset($settings->slideshow_items) && is_array($settings->slideshow_items))
		{
			...
			{
				shuffle($settings->slideshow_items);
			}
			...
			foreach ($settings->slideshow_items as $item_key => $item_value)
			{
				...
				if (!$image_in_column)
				{
					...
					if (isset($item_value->slideshow_inner_items) && is_array($item_value->slideshow_inner_items))
					{
						foreach ($item_value->slideshow_inner_items as $inner_item_key => $inner_value)
						{
							...
							$alt_text = isset($image_content->alt) ? $image_content->alt : $title_content_title;
							//Button
							...
							elseif ($content_type == 'image_content')
							{
								...
								$output .= '<img src="' . $image_content_src . '" alt="' . $alt_text . '"/>';
								$output .= '</div>';
							}
							elseif ($content_type == 'btn_content')
							{
								$output .= '<a id="' . $inner_uniqid . '" ' . (stripos($button_target, '_blank') !== false ? $button_target : '') . ' class="sppb-sp-slider-button' . $content_class . '" href="' . $button_url . '" data-layer="true" data-animation=\'{' . $animation_settings . '}\'>';
								$output .= $btn_content;
								$output .= '</a>';
							}
					...
					else
					{
						...
						if (isset($item_value->slideshow_inner_items) && is_array($item_value->slideshow_inner_items))
						{
							foreach ($item_value->slideshow_inner_items as $inner_item_key => $inner_value)
							{
								...
								{
									$output .= '<div id="' . $inner_uniqid . '" class="sppb-sp-slider-image' . $content_class . '" data-layer="true" data-animation=\'{' . $animation_settings . '}\'>';
									if ($image_content_src)
									{
										if (strpos($image_content_src, "http://") !== false || strpos($image_content_src, "https://") !== false)
										{
											$output .= '<img src="' . $image_content_src . '" alt="' . $alt_text . '">';
										}
										else
										{
											$output .= '<img src="' . Uri::base() . $image_content_src . '" alt="' . $alt_text . '">';
										}
									}
				...
				$slider_img_alt = isset($slider_img->alt) ? $slider_img->alt : '';
				...
				if ($slider_img_src)
				{
					...
					if (!empty($slider_img_alt))
					{
						$output .= '<span class="sr-only" role="img" aria-label="' . $slider_img_alt . '"></span>';
					}
				}
	...
	public function css()
	{
		...
		$transformCss = $cssHelper->generateTransformStyle('.sppb-addon-sp-slider', $settings, 'transform');
		...
		if ($content_container_option !== 'bootstrap')
		{
			$css .= $cssHelper->generateStyle('.sp-slider .sp-slider-content-wrap', $settings, ['content_container_width' => 'margin: 0 auto; width'], ['content_container_width' => '%'], [], ['content_container_width' => (isset($settings->content_container_width) && !empty($settings->content_container_width) ? $settings->content_container_width : '') . '%']);
		}
	...
	public static function getTemplate()
	{
		...
		#>
			<# _.each (data.slideshow_items, function(item_value, item_key) {
				...
				if(slider_img.src && typeof slider_img.src === "string"){
			#>
		...
		$output .= '<#
						...
						if(image_content.src){
							if(image_content.src.indexOf("http://") == 0 || image_content.src.indexOf("https://") == 0){
								...
								image_content_style += `background-image:url(${pagebuilder_base + image_content.src});background-size:100% 100%;background-position:center center;background-attachment:scroll;background-repeat: no-repeat;`;
							}
						}
						#>
		...
		$output .= $lodash->generateTransformCss('.sppb-addon-sp-slider', 'data.transform');
		$output .= '
		</style>
