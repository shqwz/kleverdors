...
class HelixUltimate
{
		...
		if ($this->params->get('scssoption'))
		{
			...
			if ($forceCompile || $needsCompile)
			{
				...
				if (file_exists($scss_path . '/' . $scss . '.scss'))
				{
					...
					$scssCache['vars'] = $vars;
					// scssphp can return an empty included-files list; remember the
					// newest .scss mtime so the cache stays usable in that case.
					$scssCache['scss_mtime'] = $this->newestScssMtime($scss_path);
	...
	public function needScssCompile($scss, $vars = array())
	{
    	...
    	/**
    	 * No imports were recorded (scssphp returned an empty list). Without a
    	 * fallback this returns true every time, so the compiled CSS is rebuilt
    	 * on every single request. Compare the stored newest .scss mtime instead:
    	 * the cache stays valid until a .scss file actually changes.
    	 */
    	if (isset($cache_file->scss_mtime)) {
    	    $scssDir = JPATH_THEMES . '/' . $this->template->template . '/scss';

    	    if ((int) $cache_file->scss_mtime === $this->newestScssMtime($scssDir)) {
    	        return false;
    	    }
    	}
	...
	/**
	 * Newest modification time across every .scss file in a directory tree.
	 *
	 * @param	string	$dir	The scss directory.
	 *
	 * @return	integer	The newest mtime, or 0 when there is nothing to read.
	 */
	private function newestScssMtime($dir)
	{
		$newest = 0;
